...
const AddSongSheet = ({ open, onClose }: Props) => {
  ...
  const renderItem: ListRenderItem<SongProps> = useCallback(({ item }) => <SongItem item={item} />, [])

  const filteredSongs = useMemo(() => {
    return playlist.filter((song) => song.title.toLowerCase().includes(search.toLowerCase()))
  }, [search])
